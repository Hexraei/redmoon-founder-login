...
// PASTE YOUR GOOGLE CLOUD AND SHEET DETAILS HERE
...
async function handleAuthResponse(response) {
    ...
    try {
        ...
        if (isFounder) {
            ...
            // 4. If not a founder, show an error and sign them out
...
async function checkIfFounder(email) {
    try {
        ...
        console.error("Error reading from Founders sheet:", err.result.error.message);
        return false;
    }
}
...
logoutBtn.addEventListener('click', async () => {
    if (Object.keys(activeTask).length > 0) {
        if (!confirm("You have an active task. Are you sure you want to logout? The active task will not be saved.")) {
            return;
        }
    }
    ...
    const tasksString = sessionData.tasksCompleted
        .map(t => `${t.name} (Time: ${t.timeTaken})`)
        .join('; ');

    const valuesToAppend = [
        sessionData.founderName,
        sessionData.loginTime.toLocaleString(),
        tasksString || "No tasks completed.",
        sessionData.logoutTime.toLocaleString(),
    ];
    ...
    try {
        ...
        // Reset the UI for the next user
        ...
        console.error("Error writing to AttendanceLog sheet:", err.result.error.message);
